...
移动端 GPU 资源紧张时,WebGL context 可能被系统回收。**必须**监听 `webglcontextlost` / `webglcontextrestored` 事件,否则玻璃效果在 context 丢失后无法自恢复:

```javascript
canvas.addEventListener('webglcontextlost', (e) => {
  e.preventDefault();        // 阻止默认行为,允许后续恢复
  renderer.dispose();        // 释放 GPU 资源,避免泄漏
  console.warn('WebGL context lost — glass effect paused');
});
canvas.addEventListener('webglcontextrestored', () => {
  initRenderer();            // 重建场景(几何体 + 材质 + shader)
  console.info('WebGL context restored — glass effect resumed');
});
```
