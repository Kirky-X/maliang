...
| redesign   | 9 维度审计清单(见 [`redesign.md`](../commands/redesign.md))逐项扫描   |
...
### 4.1 违禁省略模式(事前禁止,非事后识别)

下列模式一旦出现在产出中即视为截断,不可交付;生成时就不得使用:

| 模式         | 示例                                                                | 处置                     |
| ------------ | -------------------------------------------------------------------- | ------------------------- |
| 代码块省略注释 | `// ...` / `// rest of code` / `// TODO` / `// 其余类似`             | 补全,或显式拆分为多次产出 |
| 行话式省略   | "for brevity" / "similarly for the remaining" / "依此类推" / "同理可得" | 逐项写全                  |
| placeholder 凑数 | `#TODO` / `#PLACEHOLDER` / ` TBD`                                    | 填真实值                  |

### 4.2 交付物计数锁定

动笔前先**数清本次要交付几件**(几个文件 / 几个章节 / 几个组件变体),写成显式清单;产出完成后**核对件数**——件数不足即未完成,不可交付。这把"感觉写完了"换成"数出来齐了"。

### 4.3 PAUSED 续传协议

长输出逼近上限时,在**干净断点**(章节边界 / 文件边界)收尾当前段,然后输出续传标记:

```
[PAUSED — X of Y complete. Send "continue" to proceed.]
```

- X / Y 用实际件数(与 4.2 的计数锁定对应)
- 续传时**零复述**:不重复已交付内容,从断点直接续写
- 禁止为"塞进最后一点"而压缩剩余章节——那是 1.4 的主动收尾劣化

## 5. 防幻觉契约(参考层无命中时)

> 在 references 参考层检索 / 推导无命中时的裁决规则。来源:ui-ux-pro-max(0 结果防幻觉契约)。

- 参考层检索无命中时,禁止把一般知识**伪造成 maliang 库内规则**
- 此时仍可基于通用默认继续工作,但任何引用须显式标注:**"非库内来源,通用默认"**
- 禁止把"无命中"表述为"已按库内规范执行"——宁可声明默认值来源,不可虚构出处
